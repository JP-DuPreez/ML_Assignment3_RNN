...
class JordanRNN(nn.Module):
    ...
    Recurrence:
        h_t = activation(W_ih x_t + W_yo y_{t-1} + b_h)
        y_t = W_ho h_t + b_o

    The difference from Elman is that the feedback comes from the previous
    output rather than the previous hidden state.
    """
    ...
    def reset_parameters(self) -> None:
        for module in (self.input_to_hidden, self.output_to_hidden, self.hidden_to_output):
            if isinstance(module, nn.Linear):
                nn.init.xavier_uniform_(module.weight)
                nn.init.zeros_(module.bias)

    def init_output(self, batch_size: int, device: Optional[torch.device] = None) -> Tensor:
        """Create a zero-initialized previous output (batch, output_size)."""
    ...
    def forward(
        ...
        *,
        ...
        initial_output: Optional[Tensor]
            Initial previous output y_{-1} of shape (batch, output_size). If None, zeros are used.
        batch_first: bool
        ...
        for t in range(seq_len):
            xt = x[:, t, :]
            hidden = self.activation(self.input_to_hidden(xt) + self.output_to_hidden(prev_output))
            hidden = self.dropout(hidden)
            yt = self.hidden_to_output(hidden)
            outputs.append(yt.unsqueeze(1))
            prev_output = yt.detach()  # feedback from output; detach to prevent exploding grads through time

        outputs_tensor = torch.cat(outputs, dim=1)

        if batch_first:
            return outputs_tensor, hidden  # type: ignore[return-value]
        else:
            return outputs_tensor.transpose(0, 1), hidden  # type: ignore[return-value]
